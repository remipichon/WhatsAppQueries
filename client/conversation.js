...
ConversationHelper.prototype.parseRows = function(options) {
  ...
  // _.each(rows, function(row) {
    ...
    if (row.substring(10 + deltaD + deltaM, 11 + deltaD + deltaM) === ":") { //case hhour is one digit
      deltaH = -2;
    }
    if (row.substring(10 + deltaD + deltaM, 11 + deltaD + deltaM) !== ":") {
      deltaH = -1;
    }
    ...
    if (isMultiple || isNaN(tempDate.day) || isNaN(tempDate.hour) || isNaN(tempDate.minutes)) {
      ...
      userName = row.substring(18 + deltaM + deltaD + deltaH, header.length + 1);
